...
export default function UpdateStatus({ user }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    axios.put(`http://localhost:5000/api/employees/${user.id}`, { status })
  ...
  return (
    <form onSubmit={handleSubmit} className="p-8 max-w-md mx-auto bg-white shadow-lg rounded-lg">
      ...
      <input type="text" className="border p-2 w-full mb-4" value={status} onChange={(e) => setStatus(e.target.value)} />
